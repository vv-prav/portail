...
- **Admin** (`/admin`) — tableau de bord complet : vue d'ensemble, comptes, un panneau par jeu multijoueur (Perudo, Petit Bac, Infiltré, Yams, Motus Party), grilles/dictionnaire des mots fléchés, système (sauvegarde JSON, purge, administrateurs). Recherche transversale (comptes + historique des parties) depuis l'accueil. Le bouton **« Tirer un nouveau mot »** de l'onglet Motus fait vraiment changer le mot du jour : le tirage étant déterministe sur la date (`motusHashSeed('motus|' + date)`), supprimer la clé ne suffisait pas — un compteur `motus:variante:<date>` décale la graine, et la route insiste jusqu'à obtenir un mot différent. À zéro, la graine est identique à l'ancienne : **aucune date passée ne change de mot**. `motusWord()` et `motusWordPreview()` lisent la même graine, sinon l'aperçu « Mots à venir » annoncerait autre chose que ce qui sera tiré.
  ...
  - **Panneaux par jeu du jour** : Motus, Mots Fléchés, Le compte est bon et Géographie (deux modes) ont chacun contenu du jour, statistiques observées, modération du classement et bouton de régénération. **Les trois boutons de régénération reposent sur un compteur `<app>:variante:<date>`** : le tirage étant déterministe sur la date, supprimer la clé ne suffit pas — c'est le même piège corrigé trois fois.
...
✅ Admin · ✅ Mots Fléchés · ✅ Hub Motus · ✅ Motus du jour · ✅ Motus Party · ✅ Petit Bac · ✅ Infiltré · ✅ Yams · ✅ Le salon (`public/index.html`)
...
- **Le Mot Juste a été entièrement retiré** (septembre 2026), à la demande explicite de l'utilisateur : le moteur, le vocabulaire, la page, les routes `/api/juste/*`, le panneau d'administration, la carte du profil, le panneau « Aujourd'hui », l'enchaînement, le calendrier d'assiduité, le classement du Salon, le préchargement et le titre « Devin » (le seul qui en dépendait). Le titre « Curieux » citait « les trois jeux du jour » : il compte maintenant **trois jeux du jour différents**, quels qu'ils soient — le salon en a cinq, nommer les jeux dans un titre les fige. ⚠️ **Les données ont été effacées, définitivement** : `effacerLeMotJuste()` (server.js) balaie les clés `mj:*` une seule fois au démarrage, avec un drapeau `menage:motjuste` pour ne jamais recommencer. Sans ça elles seraient restées en base, illisibles par personne mais bien présentes dans le poids des données. La famille `menage:*` est déclarée dans `CONNUES` (admin) pour ne pas être signalée comme orpheline.
...
- **Les jeux du jour font 90 % de l'activité** : Motus 186 clés (20 joueurs), Mots Fléchés 74 (14 joueurs).
