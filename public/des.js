// =====================================================================
//  LES DÉS DU SALON — catalogue, rendu, et skin choisi
//
//  Le catalogue des skins et le dessin d'une face vivaient dans
//  `public/yams/app.js`. Perudo en avait besoin à son tour : les recopier
//  aurait donné deux catalogues à tenir à jour, qui auraient fini par
//  diverger — c'est exactement ce qui s'était produit entre les deux
//  fonctions du vocabulaire Motus.
//
//  Un seul fichier, donc, chargé par les deux jeux ET par la page
//  « Style des jeux » du profil. Un skin débloqué au Yams s'applique au
//  Perudo, et réciproquement : c'est TON dé, pas celui d'un jeu.
//
//  ⚠️ La clé de stockage reste `yams_dice_skin`, telle quelle. La renommer
//  ferait perdre son dé à tous ceux qui en avaient choisi un — pour le seul
//  plaisir d'un nom plus juste.
//
//  Usage :
//      Des.face(4)                  → le SVG d'un dé de 4, au skin courant
//      Des.face(4, { brut: true })  → sans skin (icône au trait)
//      Des.catalogue()              → { id: {name, bg, pip, ...} }
//      Des.skin() / Des.choisir(id) → lire / poser le skin
// =====================================================================
(function () {
    if (window.Des) return;
    ...
    const CLE = 'yams_dice_skin';
    let courant = null;
    try { courant = localStorage.getItem(CLE); } catch (e) {}
    if (!DICE_SKINS[courant]) courant = 'classic';

    // LE dessin d'une face — un seul pour tout le salon.
    //
    // Le Yams et le Perudo en avaient chacun un, avec des coquilles
    // différentes (corps en parchemin et reflet d'un côté, fond de skin de
    // l'autre). Deux rendus pour le même objet, c'est deux endroits où
    // corriger le jour où un skin s'affiche mal. Tout passe ici.
    //
    // `brut` donne l'icône au trait, celle des en-têtes de la feuille de
    // Yams : pas un dé posé sur la table, un pictogramme dans du texte.
    function face(n, opts) {
        const o = opts || {};
        const pips = PIP_LAYOUTS[n] || [];
        const classe = o.classe ? ' ' + o.classe : '';
        if (o.brut) {
            return `<svg viewBox="0 0 100 100" class="de-face brut${classe}" aria-hidden="true">
        ...
        const skin = DICE_SKINS[o.skin || courant] || DICE_SKINS.classic;
        const bord = skin.border ? `box-shadow:inset 0 0 0 2px ${skin.border.split(' ').slice(2).join(' ')};` : '';
        // ⚠️ La couleur des points est posée en style EN LIGNE, pas en
        // attribut : une règle CSS d'app (le Yams en avait une) l'emporterait
        // sur un attribut `fill` et écraserait la couleur du skin.
        return `<svg viewBox="0 0 100 100" class="de-face${classe}"
                     style="background:${skin.bg};border-radius:18%;${bord}">
            <ellipse cx="30" cy="20" rx="22" ry="10" class="de-reflet"/>
            ${pips.map(([x, y]) => `<circle cx="${x}" cy="${y}" r="9" style="fill:${skin.pip}"/>`).join('')}
        </svg>`;
    }

    function choisir(id) {
        if (!DICE_SKINS[id]) return false;
        courant = id;
    ...
    window.Des = {
        face,
        // La disposition des points, pour les jeux qui dessinent leur propre
        // coquille : le dé du Yams a un corps et un reflet que celui du
        // Perudo n'a pas, et unifier le RENDU serait un changement d'aspect
        // que personne n'a demandé. On partage donc le catalogue et les
        // points ; chacun garde son dessin.
        pips: (n) => PIP_LAYOUTS[n] || [],
        choisir,
        skin: () => courant,
        catalogue: () => DICE_SKINS,
        CLE,
    };
})();

/* Le reflet du dé : discret, et posé par le composant pour que les deux
   jeux aient exactement le même. */
(function () {
    if (document.getElementById('des-style')) return;
    const s = document.createElement('style');
    s.id = 'des-style';
    s.textContent = '.de-face .de-reflet{fill:#fff;opacity:.22}.de-face.brut .de-reflet{display:none}';
